...
impl CpuFftBackend {
    ...
    unsafe fn run_requested(
        ...
        let fft_w = c_uint::try_from(next_smooth_fft_size(
            u64::from(width) + 4 * u64::from(radius),
        ))
        .map_err(|_| "FFT width exceeded uint32".to_string())?;
        let fft_h = c_uint::try_from(next_smooth_fft_size(
            u64::from(height) + 4 * u64::from(radius),
        ))
...
fn fft2_in_place(plan: &FftPlan, data: &mut [Complex32], scratch: &mut [Complex32], inverse: bool) {
