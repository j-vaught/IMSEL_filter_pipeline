...
#let directional-kernel(theta-deg) = {
  let theta = theta-deg * 1deg
  let ct = calc.cos(theta)
  let st = calc.sin(theta)
  let A = ()
  for i in range(coords.len()) {
    let x0 = coords.at(i).at(0)
    let y0 = coords.at(i).at(1)
    let x = x0 * ct + y0 * st
    let y = -x0 * st + y0 * ct
    A.push(basis(degree, x, y))
  }
  ...
  for rr in range(N) {
    let row = ()
    for cc in range(N) { row.push(0.0) }
    K.push(row)
  }
  for i in range(coords.len()) {
    let x = coords.at(i).at(0)
    let y = coords.at(i).at(1)
    K.at(int(y + h)).at(int(x + h)) = P.at(1).at(i)
  }
  K
}

#let kernels = orientations.map(theta => (theta: theta, K: directional-kernel(theta)))
...
#let global-max = {
  ...
  for item in kernels {
    for rr in range(N) {
      for cc in range(N) {
        let v = calc.abs(item.K.at(rr).at(cc))
        if v > mx { mx = v }
      }
    }
  }
...
#let draw-panel(ox, oy, data) = {
  ...
  for rr in range(N) {
    for cc in range(N) {
      let x = ox + cc * cell
      let y = oy - rr * cell
      let fc = if in-circle(rr, cc) { weight-fill(data.K.at(rr).at(cc)) } else { white }
      rect((x, y), (x + cell, y - cell), fill: fc, stroke: 0.14pt + black30)
    }
  }
  ...
  content((ox + panel-w / 2.0, oy + 0.38), text(fill: black90, size: 9pt, weight: "bold")[$theta = #(data.theta)#sym.degree$])
}

#cetz.canvas({
  import cetz.draw: *
  let panel-w = N * cell
  let total-w = 4 * panel-w + 3 * gap-x

  for i in range(orientations.len()) {
    let ox = i * (panel-w + gap-x)
    draw-panel(ox, 0.0, kernels.at(i))
  }
  ...
  for i in range(steps) {
    ...
    rect((x0, legend-y), (x1, legend-y - legend-h), fill: signed-color(t), stroke: none)
  }
