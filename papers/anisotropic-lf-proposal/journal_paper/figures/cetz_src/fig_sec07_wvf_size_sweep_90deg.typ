...
#let np-values = (50, 100, 150, 200, 300, 500)
#let grid-N = 37
#let cell-sz = 0.17
...
// ── WVF weights for a single orientation ─────────────────────────────────
#let wvf-data(target-Np) = {
  ...
  (stencil: stencil, radius: nbr-data.radius, max-abs: max-abs)
}

#let all-data = np-values.map(n => (Np: n,) + wvf-data(n))
...
#cetz.canvas({
  ...
  content((bar-x, ly - bar-h - 0.25),
    text(fill: black90, size: 8pt)[$-alpha_max$ (panel)])
  ...
  content((tw / 2.0, 0.55),
    text(fill: black90, size: 11pt, weight: "bold")[
      WVF Weight Maps at #math.equation(block: false, [theta = 90 degree])
    ])
})
