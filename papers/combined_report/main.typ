...
We evaluate on four edge detection benchmarks spanning a range of image content, resolution, and annotation style. @tab:datasets provides a summary. The evaluation corpus comprises 330 images with diverse content including natural scenes, urban environments, animals, and underwater imagery.
...
#figure(
  ...
  table(
    ...
    [UDED], [30], [variable], [Underwater edge detection dataset with annotated edges in marine scenes], [@bagan2021wvf],
  ...
  caption: [Summary of the four edge detection datasets used in the evaluation. The total corpus comprises 330 images spanning underwater, outdoor, and mixed-content domains.],
) <tab:datasets>
...
Several patterns are noteworthy. The best WVF configuration is $(N_p = 25, N_s = 4, d = 2)$ on UDED, BIPED v1, and BIPED v2, demonstrating remarkable consistency across three diverse datasets spanning underwater and outdoor imagery. On BSDS500, the optimum shifts to $(N_p = 100, N_s = 4, d = 2)$, likely reflecting the lower resolution ($481 times 321$ versus $1280 times 720$) and more diverse content of BSDS500 images. In all cases, $d = 2$ and $N_s = 4$ are selected, confirming the single-image findings with strong generalization.
...
On BIPED v1, a single-image DiffusionEdge evaluation yielded ODS = 0.904, substantially exceeding the best WVF (0.812 full-dataset). This gap of 9.2 ODS points demonstrates that the largest DL models capture edge semantics---suppressing texture edges, completing occluded boundaries, respecting object-level contours---in ways that a local polynomial gradient computation fundamentally cannot. The gap between DiffusionEdge (100M parameters trained on annotated data) and WVF (3 tunable hyperparameters, no training data) reflects a genuine difference in capability for clean-data edge detection.
...
This finding partially vindicates Bagan and Wang's choice of $N_p = 250$ @bagan2021wvf. On clean data, $N_p = 250$ yields substantially lower ODS than the optimum. However, at SNR = 0.3 under Gaussian noise, $N_p = 250$ is close to the optimal range, and the performance gap narrows considerably. The originally "suboptimal" parameters are in fact near-optimal for the noisy underwater conditions Bagan and Wang targeted. The physical intuition is that larger support regions average over more pixels, providing built-in noise smoothing at the cost of reduced spatial resolution---a trade-off that is unfavorable on clean data but essential under noise.
...
The LF's noise resilience stems from its line-averaging mechanism: by fitting polynomials along multiple radial lines and combining the directional estimates, the LF effectively averages over more independent noise samples than the WVF's circular support. The reversal is modest in magnitude (typically 0.002--0.009 ODS) and occurs only at low SNR (below approximately 1.0). At moderate noise levels (SNR $>= 2.0$), WVF maintains its clean-data advantage. This finding provides additional context for Bagan and Wang's emphasis on the Line Filter: in noisy underwater environments, the line-averaging mechanism offers a genuine, if small, noise resilience advantage.
...
For underwater applications---Bagan and Wang's target domain---speckle noise is a dominant degradation mechanism @goodman1976speckle. The WVF maintains particularly strong performance under speckle noise: at SNR = 1.0, the best WVF achieves ODS = 0.831 on BSDS500, compared to NBED's 0.514 and PiDiNet's 0.511. This $0.3+$ ODS advantage under speckle noise at moderate SNR is practically significant and supports the use of parametrically-tuned WVF for underwater edge detection.
...
The pairwise Spearman correlations in @fig:spearman reveal the structure behind the aggregate concordance. The three non-BSDS datasets---UDED, BIPED v1, and BIPED v2---form a tight cluster with pairwise $rho$ between 0.96 and 0.99 ($p < 10^(-10)$ for all). These correlations are remarkably high: essentially, if a WVF configuration ranks well on any one of these three datasets, it ranks well on all three. The rank-order of all 1,206 configurations is nearly identical across underwater (UDED) and high-resolution outdoor scenes (BIPED v1 and v2), despite the substantial differences in image content, resolution, and annotation style between these datasets.
...
The noise models employed are entirely synthetic. Real imaging noise, particularly underwater noise, exhibits complex spatial correlations, non-stationary properties, and mixed degradation mechanisms not fully captured by the independent identically-distributed theoretical models used here. Underwater environments additionally suffer from wavelength-dependent absorption, backscatter, and turbidity effects that alter both signal and noise characteristics in ways our models do not capture @schettini2010underwater. Field validation on real noisy imagery from underwater, medical, and industrial domains would substantially strengthen the conclusions.
