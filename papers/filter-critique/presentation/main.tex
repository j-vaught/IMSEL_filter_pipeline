...
\begin{frame}{CPU vs. GPU Performance}
  Benchmarked CPU rate: \textbf{1,365 $\mu$s/pixel} ($N_p\!=\!15$, $N_s\!=\!18$, $d\!=\!4$, 32$\times$32 crop).

  \vspace{0.3em}
  \centering
  \begin{tabular}{lrrr}
    \toprule
    \textbf{Image} & \textbf{CPU (est.)} & \textbf{GPU} & \textbf{Speedup} \\
    \midrule
    BSDS500 ($481 \times 321$) & 3.5 min  & 0.009 s & $23{,}000\times$ \\
    BIPED ($1280 \times 720$)  & 21 min   & 0.036 s & $35{,}000\times$ \\
    \bottomrule
  \end{tabular}
  ...
  \flushleft
  \begin{itemize}
    \item CPU: triple-nested loop --- each pixel solves $N_s$ independent \texttt{np.linalg.lstsq} calls
    \item GPU: precomputed pseudoinverses + single batched matmul per orientation
    \item GPU--CPU numerical agreement: relative error $< 10^{-6}$
    \item Full ablation (453,420 configs): \textbf{12.4 hours} on GPU vs. $\sim$\textbf{18 years} on CPU
  \end{itemize}
\end{frame}
...
\begin{frame}{Dataset Samples}
  \centering
  \includegraphics[width=\textwidth, height=0.75\textheight, keepaspectratio]{figures/fig_dataset_samples.png}

  Four benchmarks spanning natural scenes (BSDS500), high-resolution outdoor imagery (BIPED v1/v2), and diverse edge targets (UDED). Total: 330 images.
\end{frame}
