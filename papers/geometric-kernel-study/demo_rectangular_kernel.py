"""
Rectangular Gaussian oriented-filter edge detection demo.

Defines a rotatable rectangular region with a 2D Gaussian distribution
applied inside the rectangle, multiplied by a first-derivative profile
in the normal direction. No virtual pixels or polynomial fitting.
"""
...
def build_rectangular_kernel(theta, half_along=6.0, half_across=3.6,
                             sigma_along=2.0, sigma_across=1.2,
                             length=15):
    """Build an oriented edge kernel using a rectangular region with Gaussian.
    ...
    half_along : float
        Half-width of the rectangle along the edge direction.
    half_across : float
        Half-width of the rectangle perpendicular to the edge.
    sigma_along : float
        Gaussian std along the edge direction.
    sigma_across : float
        Gaussian std perpendicular to the edge.
    length : int
    ...
    # Rectangular mask: hard boundary
    mask = (np.abs(u) <= half_along) & (np.abs(v) <= half_across)

    # Gaussian weighting inside the rectangle
...
def anisotropic_edge_detect(gray, n_orientations=36, half_along=6.0,
                            half_across=3.6, sigma_along=2.0,
                            sigma_across=1.2, length=15):
    """Run the oriented rectangular filter bank and pick max response."""
    ...
    for k, theta in enumerate(thetas):
        kernel = build_rectangular_kernel(theta, half_along, half_across,
...
def main():
    ...
    # Match aspect ratio ~1.67 from original filter
    sigma_along = 2.0
    sigma_across = 1.2
    half_along = 3.0 * sigma_along    # 6.0
    half_across = 3.0 * sigma_across  # 3.6
    aspect_ratio = sigma_along / sigma_across

    magnitude, angle, _ = anisotropic_edge_detect(
        gray, n_orientations=36,
        half_along=half_along, half_across=half_across,
    ...
    fig.suptitle(
        f"Rectangular kernel with Gaussian weighting  "
        f"(aspect ratio {aspect_ratio:.2f})",
        fontsize=12, y=1.02
    )
    plt.tight_layout()
    plt.savefig("edge_rectangular.png", dpi=200, bbox_inches="tight")
    plt.close()

    # Save standalone magnitude
    mag_img = Image.fromarray((mag_norm * 255).astype(np.uint8), mode="L")
    mag_img.save("edge_magnitude_rectangular.png")

    print(f"Rectangular kernel: sigma_along={sigma_along}, "
          f"sigma_across={sigma_across}, aspect={aspect_ratio:.2f}")
    print(f"Rectangle half-widths: along={half_along}, across={half_across}")
    print("Saved: edge_rectangular.png, edge_magnitude_rectangular.png")
