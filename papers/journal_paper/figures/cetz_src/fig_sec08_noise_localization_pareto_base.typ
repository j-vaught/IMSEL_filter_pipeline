...
#let render(data-path) = {
  ...
  cetz.canvas({
    ...
    content((ox + pw / 2, oy + ph + 0.48), text(fill: black70, size: 8.0pt)[Clean FWHM versus white-noise gain. WVF trace varies $r$.])
    ...
    for idx in range(trace.len() - 1) {
      let a = trace.at(idx)
      let b = trace.at(idx + 1)
      line(
    ...
    for row in trace {
      circle((tx(row.at("white_noise_gain")), ty(row.at("fwhm"))), radius: 0.06, fill: garnet, stroke: none)
    }
