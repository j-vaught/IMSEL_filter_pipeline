...
#let wvf-trace-chart(data, metric-key, snr-slug, title, y-label) = {
  ...
  cetz.canvas({
    ...
    for idx in range(points.len()) {
      ...
      content(
        (x, oy - 0.18),
        text(fill: black70, size: 5.8pt)[r=#radius-label(points.at(idx).at("radius"))],
        anchor: "north",
      )
    }
    ...
    content((ox + w / 2, oy - 0.44), text(fill: black90, size: 6.5pt)[WVF radius])
    content((0.14, oy + h / 2), angle: 90deg, text(fill: black90, size: 6.5pt)[#y-label])
  })
}
...
#let wvf-grid-heatmap(data, title) = {
  ...
  cetz.canvas({
    ...
    rect((ox, oy), (ox + w, oy + h), stroke: 0.45pt + black30)
    content((ox + w / 2, oy + h + 0.25), text(fill: black90, size: 8.0pt, weight: "bold")[#title])
    ...
    content((ox + w / 2, oy - 0.42), text(fill: black90, size: 6.5pt)[WVF radius])
    content((0.15, oy + h / 2), angle: 90deg, text(fill: black90, size: 6.5pt)[Polynomial degree])
  })
}
  ...
  if "wvf_trace" in data {
    ...
    [
      #align(center)[
        #text(fill: black90, size: 11pt, weight: "bold")[#title]
        #linebreak()
        #text(fill: black70, size: 8pt)[WVF radius-trace follow-up on the fixed BSDS crop set.]
        #linebreak()
  ...
  if "wvf_grid" in data {
    ...
    [
      #align(center)[
        #text(fill: black90, size: 11pt, weight: "bold")[#title]
        #linebreak()
        #text(fill: black70, size: 8pt)[Fine-grained WVF radius and degree sweep on the fixed BSDS crop set.]
        #linebreak()
        #text(fill: black70, size: 6.8pt)[The heatmap shows 10 dB gradient-vector RMSE across the feasible $(r,d)$ grid. Crossed cells were skipped by the Section 7 conditioning gate. The outlined cell is the annotated optimum.]
      ]
      #v(8pt)
      #wvf-grid-heatmap(data, [10 dB gradient-vector RMSE across the feasible WVF grid])
      #v(10pt)
      #align(left)[
        ...
        #text(fill: black90, size: 7.2pt)[ with RMSE #fmt(data.at("wvf_grid").at("annotated_optimum").at("value"), digits: 6).]
        #linebreak()
