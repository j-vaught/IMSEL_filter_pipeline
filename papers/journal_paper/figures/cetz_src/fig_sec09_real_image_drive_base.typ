  ...
  for row in images {
    cells.push(
      stack(
        spacing: 3pt,
        align(center)[
          text(fill: black90, size: 6.5pt, weight: "bold")[row.at("image_id")]
...
#let metric-chart(data, metric-key, title, y-label) = {
  ...
  cetz.canvas({
    ...
    for tick in y-ticks {
      ...
      content((ox - 0.12, y), text(fill: black70, size: 6pt)[fmt(tick)], anchor: "east")
    }
    for method in order {
      let method-data = methods.at(method)
      for idx in range(3) {
    ...
    content((ox + w / 2, oy - 0.44), text(fill: black90, size: 6.7pt)[Input SNR])
...
#let summary-table(data) = {
  ...
  cells.push([Metric])
  for method in order {
    cells.push([text(fill: method-color(method), weight: "bold")[methods.at(method).at("label")]])
  }
  ...
  cells.push([Ang. MAE clean])
  for method in order { cells.push([fmt(methods.at(method).at("snr_metrics").at("inf").at("orientation_mae_deg_mean"))]) }
  cells.push([Ang. MAE 10 dB])
  for method in order { cells.push([fmt(methods.at(method).at("snr_metrics").at("10").at("orientation_mae_deg_mean"))]) }
...
#let render(data-path) = {
  ...
  [
    align(center)[
      ...
      linebreak()
      text(fill: black70, size: 6.8pt)[Five DRIVE training images. Filtering uses the green channel only. Noisy metrics average over #str(config.at("noise_draws")) AWGN draws.]
    ]
    v(8pt)
    image-grid(data, [Gradient magnitude, clean retinal images], "magnitude_path", show-input: true)
  ]
  ...
  [
    ...
    v(8pt)
    image-grid(data, [Gradient orientation, clean retinal images], "orientation_path", show-input: false)
  ]
