...
def _evaluate_radius(
    ...
    def apply_case(case: StimulusCase) -> tuple[np.ndarray, np.ndarray]:
        try:
            return _apply_native(case.image, int(radius), int(degree), fft_backend, device_index)
        except Exception as exc:
            ...
            return _apply_scipy(case.image, kernel_x, kernel_y)

    for orientation_deg in orientation_values:
        for phase_px in phase_values:
            case = _step_case(xx, yy, float(orientation_deg), float(phase_px), int(radius))
            gx, gy = apply_case(case)
