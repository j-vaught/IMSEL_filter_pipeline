"""
Noise robustness vs kernel scale experiment.

Sweeps kernel size (scale factor) and SNR simultaneously for all three
filter designs. Produces a multi-panel plot: one subplot per scale,
showing SNR vs ODS for each filter.
"""
...
# ── Inline filter implementations (with FFT convolution for large kernels) ───
...
def _convolve(image, kernel):
    """Use FFT convolution for large kernels, direct for small."""
    if kernel.shape[0] > 15:
        # Pad image to handle boundaries (reflect)
        ...
        # fftconvolve valid mode trims to correct size
        return result
    else:
...
def detect(gray, build_fn, build_kwargs, n_orientations=36):
    """Generic oriented filter bank detection."""
...
def main():
    ...
    # Results: {scale: {detector: [ods_per_snr]}}
    # SNR levels include clean (None) + actual levels
    ...
    for scale in SCALES:
        ...
        results[scale] = {name: [] for name in detectors}
    ...
    print(f"Saved: snr_vs_ods_by_scale.png, ods_vs_scale.png")
